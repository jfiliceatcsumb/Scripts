...
firstName=$(/usr/libexec/PlistBuddy -c "Print :IdToken:user:profile:firstName" "$jamfConnectStateLocation" 2>/dev/null)
lastName=$(/usr/libexec/PlistBuddy -c "Print :IdToken:user:profile:lastName" "$jamfConnectStateLocation" 2>/dev/null)

if [ -n "${DisplayName}" ]; then
# Upload DisplayName to Jamf Pro if it's not empty
	if [ "$currentUser" != "root" ]; then
		/usr/local/bin/jamf recon -endUsername "${DisplayName}" -realname "${firstName} ${lastName}"
	fi
	else
