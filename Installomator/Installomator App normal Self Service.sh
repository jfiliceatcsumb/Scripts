...
LOGO="{$7:-jamf}" # "mosyleb", "mosylem", "addigy", "microsoft", "ws1", "kandji", "filewave"

item="${4}" # enter the software to install
...
# dockutil variables
addToDock="${8:-0}" # with dockutil after installation (0 if not)
dockutilAppPath="${9}"
...
OTHERinstallomatorOptions="${11}"
installomatorOptions="BLOCKING_PROCESS_ACTION=prompt_user DIALOG_CMD_FILE=${dialog_command_file} DEBUG=${DEBUG}" # Separated by space
...
cmdOutput="$(${destFile} ${item} LOGO=$LOGO ${installomatorOptions} ${installomatorNotify} ${OTHERinstallomatorOptions} || true)"
checkCmdOutput "${cmdOutput}"
